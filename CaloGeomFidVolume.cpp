...
namespace Herd
{
	...
	CaloGeomFidVolumeAlgo::CaloGeomFidVolumeAlgo(const std::string &name)
		 : Algorithm{name},
			...
			_ZCaloCenter(-36.6),
			_ZCaloHeight(73.2),
			_phiXY(atan2(-(-_XSideBig + _XSideSmall), (-_YSideSmall + _YSideBig))),
			alpha(1.),
			checkext{true},
			checkint{false},
			filterenable{true}
	...
	bool CaloGeomFidVolumeAlgo::Initialize()
	{
		...
		// inclined planes
		/*
	  	plane[RefFrame::Direction::XnegYneg] = Plane(Point((-_XSideBig / 2 - _XSideSmall / 2.) / 2. + shrink, (-_YSideBig / 2 - _YSideSmall / 2.) / 2., _ZCaloCenter),
            M_PI / 2., M_PI + _phiXY);
  		plane[RefFrame::Direction::XposYneg] = Plane(Point((_XSideBig / 2 + _XSideSmall / 2.) / 2. - shrink, (-_YSideBig / 2 - _YSideSmall / 2.) / 2., _ZCaloCenter),
            M_PI / 2., 2 * M_PI - _phiXY);
  		plane[RefFrame::Direction::XnegYpos] = Plane(Point((-_XSideBig / 2 - _XSideSmall / 2.) / 2. + shrink, (_YSideBig / 2 + _YSideSmall / 2.) / 2., _ZCaloCenter),
            M_PI / 2., M_PI - _phiXY);
  		plane[RefFrame::Direction::XposYpos] = Plane(Point((_XSideBig / 2 + _XSideSmall / 2.) / 2. - shrink, (_YSideBig / 2 + _YSideSmall / 2.) / 2., _ZCaloCenter),
            M_PI / 2., _phiXY);
		*/
		...
		for (auto idir : RefFrame::Directions)
		{
			m[idir] = 0;
			q[idir] = 0;
		}
	...
	bool CaloGeomFidVolumeAlgo::CheckExt()
	{
		...
		if (!mcTruth)
		{
			COUT(ERROR) << "mcTruth not present for event " << GetEventLoopProxy()->GetCurrentEvent() << ENDL;
			return false;
		}
		...
		{
			nintXpos++;
			FillCoo(intZpos, _processstore->calofidvolxposEntry, nintXpos - 1);
		}
		...
		{
			nintXpos++;
			FillCoo(intZneg, _processstore->calofidvolxposEntry, nintXpos - 1);
		}
		if (intY6[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intY6[RefFrame::Coo::X] > +_XSideBig / 2. - alpha * cubeside &&
			 intY6[RefFrame::Coo::Z] > -_ZCaloHeight &&
			 intY6[RefFrame::Coo::Z] < 0.)
		{
			nintXpos++;
			FillCoo(intY6, _processstore->calofidvolxposEntry, nintXpos - 1);
		}
		if (intY14[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intY14[RefFrame::Coo::X] > +_XSideBig / 2. - alpha * cubeside &&
			 intY14[RefFrame::Coo::Z] > -_ZCaloHeight &&
			 intY14[RefFrame::Coo::Z] < 0.)
		{
			nintXpos++;
			FillCoo(intY14, _processstore->calofidvolxposEntry, nintXpos - 1);
		}
		if (nintXpos > 2)
		{
			COUT(ERROR) << "intXpos>2" << ENDL;
		} //}return false;}
		if (nintXpos == 2)
		{
			_processstore->calofidvolxpos = 1;
		...
		{
			nintXneg++;
			FillCoo(intZpos, _processstore->calofidvolxnegEntry, nintXneg - 1);
		}
		...
		{
			nintXneg++;
			FillCoo(intZneg, _processstore->calofidvolxnegEntry, nintXneg - 1);
		}
		...
		{
			nintXneg++;
			FillCoo(intY6, _processstore->calofidvolxnegEntry, nintXneg - 1);
		}
		...
		{
			nintXneg++;
			FillCoo(intY14, _processstore->calofidvolxnegEntry, nintXneg - 1);
		}
		if (nintXneg > 2)
		{
			COUT(ERROR) << "intXneg>2" << ENDL;
		...
		if (nintXneg == 2)
		{
			_processstore->calofidvolxneg = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		if (intZpos[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 ...
			 intZpos[RefFrame::Coo::Y] > +_YSideBig / 2. - alpha * cubeside)
		{
			nintYpos++;
			FillCoo(intZpos, _processstore->calofidvolyposEntry, nintYpos - 1);
		}
		if (intZneg[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intZneg[RefFrame::Coo::X] > -_XSideSmall / 2. &&
			 intZneg[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] > +_YSideBig / 2. - alpha * cubeside)
		{
			nintYpos++;
			FillCoo(intZneg, _processstore->calofidvolyposEntry, nintYpos - 1);
		}
		if (intX6[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intX6[RefFrame::Coo::Y] > +_YSideBig / 2. - alpha * cubeside &&
			 intX6[RefFrame::Coo::Z] > -_ZCaloHeight &&
			 intX6[RefFrame::Coo::Z] < 0.)
		{
			nintYpos++;
			FillCoo(intX6, _processstore->calofidvolyposEntry, nintYpos - 1);
		}
		if (intX14[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intX14[RefFrame::Coo::Y] > +_YSideBig / 2. - alpha * cubeside &&
			 intX14[RefFrame::Coo::Z] > -_ZCaloHeight &&
			 intX14[RefFrame::Coo::Z] < 0.)
		{
			nintYpos++;
			FillCoo(intX14, _processstore->calofidvolyposEntry, nintYpos - 1);
		}
		if (nintYpos > 2)
		{
			COUT(ERROR) << "intYpos>2" << ENDL;
		} // return false;}
		if (nintYpos == 2)
		{
			_processstore->calofidvolypos = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintYneg++;
			FillCoo(intZpos, _processstore->calofidvolynegEntry, nintYneg - 1);
		}
		if (intZneg[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intZneg[RefFrame::Coo::X] > -_XSideSmall / 2. &&
			 intZneg[RefFrame::Coo::Y] > -_YSideBig / 2. &&
		...
		{
			nintYneg++;
			FillCoo(intZneg, _processstore->calofidvolynegEntry, nintYneg - 1);
		}
		if (intX6[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intX6[RefFrame::Coo::Y] < -_YSideBig / 2. + alpha * cubeside &&
			 intX6[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintYneg++;
			FillCoo(intX6, _processstore->calofidvolynegEntry, nintYneg - 1);
		}
		if (intX14[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intX14[RefFrame::Coo::Y] < -_YSideBig / 2. + alpha * cubeside &&
			 intX14[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintYneg++;
			FillCoo(intX14, _processstore->calofidvolynegEntry, nintYneg - 1);
		}
		if (nintYneg > 2)
		{
			COUT(ERROR) << "intYneg>2" << ENDL;
		} // return false;}
		if (nintYneg == 2)
		{
			_processstore->calofidvolyneg = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		if (intYneg[RefFrame::Coo::X] > -_XSideSmall / 2. &&
			 intYneg[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intYneg[RefFrame::Coo::Z] < 0 &&
			 intYneg[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intYneg, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intYpos[RefFrame::Coo::X] > -_XSideSmall / 2. &&
			 intYpos[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intYpos[RefFrame::Coo::Z] < 0 &&
			 intYpos[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intYpos, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXneg[RefFrame::Coo::Y] > -_YSideSmall / 2. &&
			 intXneg[RefFrame::Coo::Y] < +_YSideSmall / 2. &&
			 intXneg[RefFrame::Coo::Z] < 0 &&
			 intXneg[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXneg, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXpos[RefFrame::Coo::Y] > -_YSideSmall / 2. &&
			 intXpos[RefFrame::Coo::Y] < +_YSideSmall / 2. &&
			 intXpos[RefFrame::Coo::Z] < 0 &&
			 intXpos[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXpos, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXposYneg[RefFrame::Coo::X] > +_XSideSmall / 2. &&
			 ...
			 intXposYneg[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXposYneg, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXnegYneg[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 ...
			 intXnegYneg[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXnegYneg, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXnegYpos[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 ...
			 intXnegYpos[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXnegYpos, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (intXposYpos[RefFrame::Coo::X] > +_XSideSmall / 2. &&
			 ...
			 intXposYpos[RefFrame::Coo::Z] > 0 - alpha * cubeside)
		{
			nintZpos++;
			FillCoo(intXposYpos, _processstore->calofidvolzposEntry, nintZpos - 1);
		}
		if (nintZpos > 2)
		{
			COUT(ERROR) << "intZpos>2" << ENDL;
		} // return false;}
		if (nintZpos == 2)
		{
			_processstore->calofidvolzpos = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintZneg++;
			FillCoo(intYneg, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intYpos[RefFrame::Coo::X] > -_XSideSmall / 2. &&
			 intYpos[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intYpos[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintZneg++;
			FillCoo(intYpos, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXneg[RefFrame::Coo::Y] > -_YSideSmall / 2. &&
			 intXneg[RefFrame::Coo::Y] < +_YSideSmall / 2. &&
			 intXneg[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintZneg++;
			FillCoo(intXneg, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXpos[RefFrame::Coo::Y] > -_YSideSmall / 2. &&
			 intXpos[RefFrame::Coo::Y] < +_YSideSmall / 2. &&
			 intXpos[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintZneg++;
			FillCoo(intXpos, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXposYneg[RefFrame::Coo::X] > +_XSideSmall / 2. &&
			 intXposYneg[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intXposYneg[RefFrame::Coo::Y] > -_YSideBig / 2. &&
		...
		{
			nintZneg++;
			FillCoo(intXposYneg, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXnegYneg[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intXnegYneg[RefFrame::Coo::X] < -_XSideSmall / 2. &&
			 intXnegYneg[RefFrame::Coo::Y] > -_YSideBig / 2. &&
		...
		{
			nintZneg++;
			FillCoo(intXnegYneg, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXnegYpos[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intXnegYpos[RefFrame::Coo::X] < -_XSideSmall / 2. &&
			 intXnegYpos[RefFrame::Coo::Y] > +_YSideSmall / 2. &&
		...
		{
			nintZneg++;
			FillCoo(intXnegYpos, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (intXposYpos[RefFrame::Coo::X] > +_XSideSmall / 2. &&
			 intXposYpos[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intXposYpos[RefFrame::Coo::Y] > +_YSideSmall / 2. &&
		...
		{
			nintZneg++;
			FillCoo(intXposYpos, _processstore->calofidvolznegEntry, nintZneg - 1);
		}
		if (nintZneg > 2)
		{
			COUT(ERROR) << "intZneg>2" << ENDL;
		} // return false;}
		if (nintZneg == 2)
		{
			_processstore->calofidvolzneg = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintXnegYneg++;
			FillCoo(intXneg, _processstore->calofidvolxnegynegEntry, nintXnegYneg - 1);
		}
		if (intYneg[RefFrame::Coo::X] > -_XSideSmall / 2. &&
		...
		{
			nintXnegYneg++;
			FillCoo(intYneg, _processstore->calofidvolxnegynegEntry, nintXnegYneg - 1);
		}
		if (intZpos[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] > mXNEGYNEG * intZpos[RefFrame::Coo::X] + qXNEGYNEG &&
			 intZpos[RefFrame::Coo::Y] < mXNEGYNEG * intZpos[RefFrame::Coo::X] + qXNEGYNEG + alpha * cubeside)
		{
			nintXnegYneg++;
			FillCoo(intZpos, _processstore->calofidvolxnegynegEntry, nintXnegYneg - 1);
		}
		if (intZneg[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] > mXNEGYNEG * intZneg[RefFrame::Coo::X] + qXNEGYNEG &&
			 intZneg[RefFrame::Coo::Y] < mXNEGYNEG * intZneg[RefFrame::Coo::X] + qXNEGYNEG + alpha * cubeside)
		{
			nintXnegYneg++;
			FillCoo(intZneg, _processstore->calofidvolxnegynegEntry, nintXnegYneg - 1);
		}
		if (nintXnegYneg > 2)
		{
			COUT(ERROR) << "nintXegYneg>2" << ENDL;
		} // return false;}
		if (nintXnegYneg == 2)
		{
			_processstore->calofidvolxnegyneg = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintXposYneg++;
			FillCoo(intXpos, _processstore->calofidvolxposynegEntry, nintXposYneg - 1);
		}
		if (intYneg[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intYneg[RefFrame::Coo::X] > +_XSideSmall / 2 - alpha * cubeside &&
			 intYneg[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintXposYneg++;
			FillCoo(intYneg, _processstore->calofidvolxposynegEntry, nintXposYneg - 1);
		}
		if (intZpos[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] > mXPOSYNEG * intZpos[RefFrame::Coo::X] + qXPOSYNEG &&
			 intZpos[RefFrame::Coo::Y] < mXPOSYNEG * intZpos[RefFrame::Coo::X] + qXPOSYNEG + alpha * cubeside)
		{
			nintXposYneg++;
			FillCoo(intZpos, _processstore->calofidvolxposynegEntry, nintXposYneg - 1);
		}
		if (intZneg[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] > -_YSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] > mXPOSYNEG * intZneg[RefFrame::Coo::X] + qXPOSYNEG &&
			 intZneg[RefFrame::Coo::Y] < mXPOSYNEG * intZneg[RefFrame::Coo::X] + qXPOSYNEG + alpha * cubeside)
		{
			nintXposYneg++;
			FillCoo(intZneg, _processstore->calofidvolxposynegEntry, nintXposYneg - 1);
		}
		if (nintXposYneg > 2)
		{
			COUT(ERROR) << "nintXposYneg>2" << ENDL;
		} //return false;}
		if (nintXposYneg == 2)
		{
			_processstore->calofidvolxposyneg = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintXnegYpos++;
			FillCoo(intXneg, _processstore->calofidvolxnegyposEntry, nintXnegYpos - 1);
		}
		if (intYpos[RefFrame::Coo::X] > -_XSideSmall / 2. &&
		...
		{
			nintXnegYpos++;
			FillCoo(intYpos, _processstore->calofidvolxnegyposEntry, nintXnegYpos - 1);
		}
		if (intZpos[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] < mXNEGYPOS * intZpos[RefFrame::Coo::X] + qXNEGYPOS &&
			 intZpos[RefFrame::Coo::Y] > mXNEGYPOS * intZpos[RefFrame::Coo::X] + qXNEGYPOS - alpha * cubeside)
		{
			nintXnegYpos++;
			FillCoo(intZpos, _processstore->calofidvolxnegyposEntry, nintXnegYpos - 1);
		}
		if (intZneg[RefFrame::Coo::X] > -_XSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] < mXNEGYPOS * intZneg[RefFrame::Coo::X] + qXNEGYPOS &&
			 intZneg[RefFrame::Coo::Y] > mXNEGYPOS * intZneg[RefFrame::Coo::X] + qXNEGYPOS - alpha * cubeside)
		{
			nintXnegYpos++;
			FillCoo(intZneg, _processstore->calofidvolxnegyposEntry, nintXnegYpos - 1);
		}
		if (nintXnegYpos > 2)
		{
			COUT(ERROR) << "nintXnegYpos>2" << ENDL;
		} //return false;}
		if (nintXnegYpos == 2)
		{
			_processstore->calofidvolxnegypos = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
		...
		{
			nintXposYpos++;
			FillCoo(intXpos, _processstore->calofidvolxposyposEntry, nintXposYpos - 1);
		}
		if (intYpos[RefFrame::Coo::X] < +_XSideSmall / 2. &&
			 intYpos[RefFrame::Coo::X] > +_XSideSmall / 2 - alpha * cubeside &&
			 intYpos[RefFrame::Coo::Z] > -_ZCaloHeight &&
		...
		{
			nintXposYpos++;
			FillCoo(intYpos, _processstore->calofidvolxposyposEntry, nintXposYpos - 1);
		}
		if (intZpos[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intZpos[RefFrame::Coo::Y] < mXPOSYPOS * intZpos[RefFrame::Coo::X] + qXPOSYPOS &&
			 intZpos[RefFrame::Coo::Y] > mXPOSYPOS * intZpos[RefFrame::Coo::X] + qXPOSYPOS - alpha * cubeside)
		{
			nintXposYpos++;
			FillCoo(intZpos, _processstore->calofidvolxposyposEntry, nintXposYpos - 1);
		}
		if (intZneg[RefFrame::Coo::X] < +_XSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] < +_YSideBig / 2. &&
			 intZneg[RefFrame::Coo::Y] < mXPOSYPOS * intZneg[RefFrame::Coo::X] + qXPOSYPOS &&
			 intZneg[RefFrame::Coo::Y] > mXPOSYPOS * intZneg[RefFrame::Coo::X] + qXPOSYPOS - alpha * cubeside)
		{
			nintXposYpos++;
			FillCoo(intZneg, _processstore->calofidvolxposyposEntry, nintXposYpos - 1);
		}
		if (nintXposYpos > 2)
		{
			COUT(ERROR) << "nintXposYpos>2" << ENDL;
		} // return false;}
		if (nintXposYpos == 2)
		{
			_processstore->calofidvolxposypos = 1;
			_processstore->calofidvolpass = false;
			SetFilterResult(FilterResult::REJECT);
		}
		else
	...
	bool CaloGeomFidVolumeAlgo::CheckInt()
	{
		...
		if (intXpos[RefFrame::Coo::Y] < pxy[1][RefFrame::Coo::Y] &&
			 ...
			 intXpos[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxpos = 1; //COUT(INFO)<<"XPOS"<<ENDL;
		}												  //FillCoo(intXpos,_processstore->calofidvolxposEntry,nintXpos-1); }
		...
		if (intXneg[RefFrame::Coo::Y] < pxy[6][RefFrame::Coo::Y] &&
			 ...
			 intXneg[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxneg = 1; //COUT(INFO)<<"XNEG"<<ENDL;
		}												  //FillCoo(intXneg,_processstore->calofidvolxnegEntry,nintXneg-1); }
		...
		if (intYpos[RefFrame::Coo::X] < pxy[0][RefFrame::Coo::X] &&
			 ...
			 intYpos[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolypos = 1; //COUT(INFO)<<"YPOS"<<ENDL;
		}												  //FillCoo(intYpos,_processstore->calofidvolyposEntry,nintYpos-1); }
		...
		if (intYneg[RefFrame::Coo::X] < pxy[3][RefFrame::Coo::X] &&
			 ...
			 intYneg[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolyneg = 1; //COUT(INFO)<<"YNEG"<<ENDL;
		}												  //FillCoo(intYneg,_processstore->calofidvolynegEntry,nintYneg-1); }
		...
		{
			nint++;
			_processstore->calofidvolzpos = 1; // COUT(INFO)<<"ZPOS"<<ENDL;
		}												  // FillCoo(intZpos,_processstore->calofidvolzposEntry,nintZpos-1); }
		//BOTTOM CAP CHECK: ZNEG
		// Assume an octagon
		...
		if (intZneg[RefFrame::Coo::X] < pxy[1][RefFrame::Coo::X] &&
			 ...
			 intZneg[RefFrame::Coo::Y] > m[RefFrame::Direction::XnegYneg] * intZneg[RefFrame::Coo::X] + q[RefFrame::Direction::XnegYneg])
		{
			nint++;
			_processstore->calofidvolzneg = 1; //COUT(INFO)<<"ZNEG"<<ENDL;
		}												  // FillCoo(intZneg,_processstore->calofidvolznegEntry,nintZneg-1); }
		...
		if (intXnegYneg[RefFrame::Coo::X] > pxy[5][RefFrame::Coo::X] &&
			 ...
			 intXnegYneg[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxnegyneg = 1; //COUT(INFO)<<"XNEG-YNEG"<<ENDL;
		}														// FillCoo(intXnegYneg,_processstore->calofidvolxnegynegEntry,nintXnegYneg-1); }
		...
		if (intXposYneg[RefFrame::Coo::X] > pxy[3][RefFrame::Coo::X] &&
			 ...
			 intXposYneg[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxposyneg = 1; //COUT(INFO)<<"XPOS-YNEG"<<ENDL;
		}														// FillCoo(intXposYneg,_processstore->calofidvolxposynegEntry,nintXposYneg-1); }
		...
		if (intXnegYpos[RefFrame::Coo::X] > pxy[6][RefFrame::Coo::X] &&
			 ...
			 intXnegYpos[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxnegypos = 1; //COUT(INFO)<<"XNEG-YPOS"<<ENDL;
		}														// FillCoo(intXnegYpos,_processstore->calofidvolxnegyposEntry,nintXnegYpos-1); }
		...
		if (intXposYpos[RefFrame::Coo::X] > pxy[0][RefFrame::Coo::X] &&
			 ...
			 intXposYpos[RefFrame::Coo::Z] < pz[0][RefFrame::Coo::Z])
		{
			nint++;
			_processstore->calofidvolxposypos = 1; //COUT(INFO)<<"XPOS-YPOS"<<ENDL;
		}														// FillCoo(intXposYpos,_processstore->calofidvolxposyposEntry,nintXposYpos-1); }
...
CaloGeomFidVolumeStore::CaloGeomFidVolumeStore(const std::string &name) : Algorithm{name}
{
}
